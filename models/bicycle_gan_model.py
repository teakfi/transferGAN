"""
File copied from BicycleGAN https://github.com/junyanz/BicycleGAN
...
class BiCycleGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        use_vae = True
        self.model_names = ['G']
        use_dropout = not opt.no_dropout  # different option 
        # networks renamed
        self.netG = networks.define_BG(opt.input_nc, opt.output_nc, opt.nz, opt.ngf, netG=opt.netG,
                                      norm=opt.norm, nl=opt.nl, use_dropout=use_dropout, init_type=opt.init_type, init_gain=opt.init_gain,
        ...
        if use_D:
            self.model_names += ['D']
            self.netD = networks.define_BD(D_output_nc, opt.ndf, netD=opt.netD, norm=opt.norm, nl=opt.nl,
                                          init_type=opt.init_type, init_gain=opt.init_gain, num_Ds=opt.num_Ds, gpu_ids=self.gpu_ids)
        if use_D2:
            self.model_names += ['D2']
            self.netD2 = networks.define_BD(D_output_nc, opt.ndf, netD=opt.netD2, norm=opt.norm, nl=opt.nl,
                                           init_type=opt.init_type, init_gain=opt.init_gain, num_Ds=opt.num_Ds, gpu_ids=self.gpu_ids)
        else:
            self.netD2 = None
        if use_E:
            self.model_names += ['E']
            self.netE = networks.define_BE(opt.output_nc, opt.nz, opt.nef, netE=opt.netE, norm=opt.norm, nl=opt.nl,
        ...
        if opt.isTrain:
            ...
            self.optimizers = []
            # optimizer learning rate options different from bicycleGAN
            self.optimizer_G = torch.optim.Adam(self.netG.parameters(), lr=opt.lrG, betas=(opt.beta1G, 0.999))
            self.optimizers.append(self.optimizer_G)
            if use_E:
                self.optimizer_E = torch.optim.Adam(self.netE.parameters(), lr=opt.lrE, betas=(opt.beta1E, 0.999))
                self.optimizers.append(self.optimizer_E)

            if use_D:
                self.optimizer_D = torch.optim.Adam(self.netD.parameters(), lr=opt.lrD, betas=(opt.beta1D, 0.999))
                self.optimizers.append(self.optimizer_D)
            if use_D2:
                self.optimizer_D2 = torch.optim.Adam(self.netD2.parameters(), lr=opt.lrD2, betas=(opt.beta1D2, 0.999))
